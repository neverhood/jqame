...
(function () {
	...
	WMDEditor.defaults = { // {{{
		...
		// entering links.
		imageDialogText: "<p style='margin-top: 0px'><b>Пожалуйста, укажите ссылку на изображение.</b></p><p>Вы также можете указать заголовок</p><p>Пример:<br />http://i.imgur.com/1cZl4.jpg</p>",
		linkDialogText: "<p style='margin-top: 0px'><b>Пожалуйста, укажите адрес.</b></p><p>Вы также можете указать заголовок.</p><p>Пример:<br />http://www.google.com/</p>",
	...
	var util = { // {{{
		...
		prompt: function (text, defaultInputText, makeLinkMarkdown, promptType) {
			...
			var createDialog = function () {
                ...
                if ( promptType == 'Image' ) {
                    var urlText = 'Адрес изображения: ',
                        hoverText = 'Заголовок: '
                }

					label.appendChild(document.createTextNode(urlText));
				...
				form.appendChild(label);
					...
					label.appendChild(document.createTextNode(hoverText));
		...
		wmd.editor = function (previewRefreshCallback) { // {{{
			...
			var makeSpritedButtonRow = function () {
				...
				for (var i=0;i<buttonlist.length;i++) {
					switch (buttonlist[i]) {
					case "bold":
						addButton("wmd-bold-button", "Жирный <strong> Ctrl+B", command.doBold);
						break;
					case "italic":
						addButton("wmd-italic-button", "Курсив <em> Ctrl+I", command.doItalic);
						break;
					case 'link':
						addButton("wmd-link-button", "Ссылка <a> Ctrl+L", function (chunk, postProcessing, useDefaultText) {
							return command.doLinkOrImage(chunk, postProcessing, false);
						});
						break;
					case 'blockquote':
						addButton("wmd-quote-button", "Цитата <blockquote> Ctrl+Q", command.doBlockquote);
						break;
					case 'code':
						addButton("wmd-code-button", "Код <pre><code> Ctrl+K", command.doCode);
						break;
					case 'image':
						addButton("wmd-image-button", "Изображение <img> Ctrl+G", function (chunk, postProcessing, useDefaultText) {
							return command.doLinkOrImage(chunk, postProcessing, true);
						});
						break;
					case 'ol':
						addButton("wmd-olist-button", "Пронумерованный список <ol> Ctrl+O", function (chunk, postProcessing, useDefaultText) {
							command.doList(chunk, postProcessing, true, useDefaultText);
						});
						break;
					case 'ul':
						addButton("wmd-ulist-button", "Маркированный список <ul> Ctrl+U", function (chunk, postProcessing, useDefaultText) {
							command.doList(chunk, postProcessing, false, useDefaultText);
						});
						break;
					case 'heading':
						addButton("wmd-heading-button", "Заголовок <h1>/<h2> Ctrl+H", command.doHeading);
						break;
					case 'hr':
						addButton("wmd-hr-button", "Горизонтальная черта <hr> Ctrl+R", command.doHorizontalRule);
						break;
					case 'undo':
						var undoButton = addButton("wmd-undo-button", "Отмена - Ctrl+Z");
						undoButton.execute = function (manager) {
							manager.undo();
						};
						break;
					case 'redo':
						var redoButton = addButton("wmd-redo-button", "Вернуть - Ctrl+Y");
						if (/win/.test(nav.platform.toLowerCase())) {
							redoButton.title = "Redo - Ctrl+Y";
						}
						else {
							// mac and other non-Windows platforms
							redoButton.title = "Вернуть - Ctrl+Shift+Z";
						}
