...
{
	/**
	 * Slide down the matching elements.
	 *
	...
	$.fn.slideDown = function(duration, easing, callback)
	{
		...
		return this.queue(function(next)
		{
			...
			// Temporarily show the element so we can correctly determine its
			// height -- this won't be visible to the user becase we're
			// animating soon after
			element.show();

			// Element properties that contribute to heights
			var properties = {
			...
			{
				// Filter out non-numeric properties, since we cannot animate
				// those
			...
			element.animate(
				properties, options.duration, options.easing,
				function()
				{
					// Remove leftover inline CSS from animation
